...
<template>
    <div>
        ...
        <div v-if="$loadingRouteData">
            Loading Topics... (add nice animation)
        </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            getUnflaggedTopics: function() {
                return this.$http.get('topics?unflagged=true', function (data, status, request) {
                    this.unflaggedTopics = data; // You can remove this is your not going to reload topics on the page and resuse the method.
                    return data;
                }).error(function (data, status, request) {
                    console.log('error', data);
                });
        ...
        route: {
            data: function({}) {
                return Promise.all([
                    this.getUnflaggedTopics()
                ]).then(([unflaggedTopics]) => ({ unflaggedTopics }))
            }
        }
    };
</script>
